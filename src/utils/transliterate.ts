// Phonetic and Script Transliteration Engine for Real-Time Lexilogos-style typing
...
// Arabic transliteration rules (Lexilogos / Arabizi / Buckwalter)
...
export const RUNES_MAP: Record<string, string> = {
  'f': 'ᚠ', 'u': 'ᚢ', 'v': 'ᚢ', 'th': 'ᚦ', 'a': 'ᚨ', 'r': 'ᚱ', 'k': 'ᚲ', 'c': 'ᚲ',
...
export const HIEROGLYPH_MAP: Record<string, string> = {
  ...
  'f': '𓆑', 'm': '𓅓', 'n': '𓈖', 'r': '𓂋', 'h': '𓉔', 'H': '𓎛', 'kh': '𓐍',
  's': '𓋴', 'sh': '𓈙', 'q': '𓈎', 'k': '𓎡', 'g': '𓎼', 't': '𓏏', 'th': '𓍿',
  'd': '𓂧', 'dj': '𓆓', ' ': '  '
};
...
export function transliterateText(input: string, keyboardId: string): string {
  ...
  if (keyboardId.includes('arabic') || keyboardId === 'persian' || keyboardId === 'urdu') {
    rules = ARABIC_RULES;
  } else if (keyboardId.includes('russian') || keyboardId.includes('cyrillic') || keyboardId === 'ukrainian' || keyboardId === 'bulgarian') {
    rules = RUSSIAN_RULES;
  } else if (keyboardId.includes('greek')) {
    rules = GREEK_RULES;
