...
export const VirtualKeyboard: React.FC<VirtualKeyboardProps> = ({
  ...
  return (
    <div className={`rounded-2xl border p-3 sm:p-5 select-none transition-colors duration-200 shadow-sm ${
      isDarkMode 
      ...
      <div className={`flex flex-wrap items-center justify-between gap-2.5 pb-3 border-b mb-3 ${
      ...
      {currentKeyboard.diacritics && currentKeyboard.diacritics.length > 0 && (
        <div className={`mb-3 p-2 rounded-xl border flex items-center gap-1.5 overflow-x-auto scrollbar-thin ${
          ...
          <div className="flex items-center gap-1.5 flex-nowrap">
            {currentKeyboard.diacritics.map((dia, idx) => (
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center font-bold text-xl rounded-lg border transition-all shrink-0 cursor-pointer ${
                  isDarkMode 
                    ? 'bg-slate-800 hover:bg-slate-700 text-red-400 border-slate-700' 
                    : 'bg-slate-50 hover:bg-red-50 text-[#a80000] border-slate-200 hover:border-red-300 shadow-2xs'
                }`}
              >
                {`◌${dia}`}
              </button>
      ...
      {viewMode === 'lexilogos' ? (
        <div className="space-y-2 sm:space-y-3 py-1">
          {rowsToRender.map((row, rowIdx) => (
            <div 
              key={`lexi-row-${rowIdx}`} 
              className="flex flex-wrap justify-center items-end gap-1.5 sm:gap-2.5"
            >
              {row.map((kDef, kIdx) => {
                const charToShow = kDef.displayChar || kDef.char;
                const hasGuide = Boolean(kDef.latinGuide);

                return (
                  <div key={`lexi-key-${rowIdx}-${kIdx}`} className="flex flex-col items-center">
                    {/* Original Keyboard Letter Guide Centered Above Key */}
                    <span 
                      className={`text-[11px] sm:text-xs font-bold tracking-wider h-4 sm:h-5 flex items-center justify-center font-mono ${
                    ...
                    {/* Keycap: White Rounded Card with Deep Crimson Script Letter */}
                    <button
                      onClick={() => handleKeyClick(kDef)}
                      title={kDef.latinGuide ? `Touche: ${kDef.latinGuide} → ${kDef.char}` : kDef.char}
                      className={`w-9 sm:w-12 md:w-13 h-10 sm:h-12 bg-white text-[#a80000] hover:text-[#dc2626] active:scale-95 rounded-lg shadow-sm border border-slate-300 hover:border-red-400 flex items-center justify-center font-bold text-xl sm:text-2xl transition-all cursor-pointer select-none ${
        ...
        <div className="space-y-1.5 sm:space-y-2">
            ...
            <div key={`row-${rowIdx}`} className="flex justify-center gap-1 sm:gap-1.5">
              ...
              {rowIdx === 3 && (
                <button
                  ...
                  className={`flex items-center justify-center gap-1 px-2.5 sm:px-4 py-2 sm:py-3 rounded-lg text-xs font-bold tracking-wider uppercase transition-all active:scale-95 cursor-pointer shadow-xs ${
                    isShiftActive 
              ...
              {row.map((kDef, kIdx) => {
                ...
                return (
                  <button
                    key={`key-${rowIdx}-${kIdx}`}
                    onClick={() => handleKeyClick(kDef)}
                    className={`relative group rounded-lg sm:rounded-xl font-medium transition-all active:scale-95 flex flex-col items-center justify-center p-1 sm:p-2 border cursor-pointer ${
                  ...
                  >
                    ...
                    {(kDef.latinGuide || kDef.shiftChar) && (
                      <span className="absolute top-0.5 left-1 text-[9px] sm:text-[10px] text-slate-500 font-mono font-semibold">
                        {kDef.latinGuide || kDef.shiftChar}
                      </span>
                    )}

                    {/* Main Key Display */}
                    <span className={`text-base sm:text-xl font-bold ${currentKeyboard.fontFamilyClass || ''}`}>
                      {displayChar}
                    </span>
                  </button>
                );
              ...
              {rowIdx === 0 && (
                <button
                  ...
                  className={`flex items-center justify-center px-2.5 sm:px-4 py-2 rounded-lg sm:rounded-xl text-xs font-bold transition-all active:scale-95 shadow-xs cursor-pointer ${
              ...
              {rowIdx === 3 && (
                <button
                  onClick={() => setIsShiftActive(!isShiftActive)}
                  className={`flex items-center justify-center gap-1 px-2.5 sm:px-4 py-2 rounded-lg text-xs font-bold tracking-wider uppercase transition-all active:scale-95 cursor-pointer shadow-xs ${
                    isShiftActive 
                      ? 'bg-emerald-600 text-white border border-emerald-600' 
          ...
          <div className="flex justify-center gap-1 sm:gap-1.5 pt-1">
            <button
              onClick={() => setIsCapsLockActive(!isCapsLockActive)}
              className={`px-3 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer shadow-xs ${
                isCapsLockActive 
                  ? 'bg-emerald-600 text-white border border-emerald-600' 
                  : (isDarkMode ? 'bg-slate-800 text-slate-300 border-slate-700' : 'bg-slate-200 text-slate-600 border border-slate-300')
              }`}
            ...
            <button
              ...
              className="px-6 py-2.5 bg-white hover:bg-slate-50 active:bg-slate-100 border border-slate-300 text-slate-500 rounded-lg sm:rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all active:scale-98 shadow-xs cursor-pointer"
              style={{ flex: 6 }}
            >
              <Space className="w-4 h-4 text-slate-400" />
            ...
            <button
              ...
              className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white border border-emerald-600 rounded-lg sm:rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition-all active:scale-95 shadow-xs cursor-pointer"
              style={{ flex: 2 }}
            >
