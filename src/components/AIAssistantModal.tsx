      ...
      if (activeTab === 'transliterate') {
        const res = await fetch('/api/ai/transliterate', {
          ...
          body: JSON.stringify({
            ...
            targetScript: currentKeyboard.name,
          }),
        });
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base flex items-center gap-2">
                ...
                <span className="text-[10px] font-semibold bg-indigo-500/40 text-indigo-200 border border-indigo-400/30 px-2 py-0.5 rounded-full">
                  Gemini Flash
                </span>
              </h3>
            ...
            <div className="space-y-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">Transliteration</span>
                  <span className="text-sm font-semibold text-slate-800">{result.transliteration || 'N/A'}</span>
                </div>

                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">IPA Phonetic Transcription</span>
                  <span className="text-sm font-mono text-indigo-700">/{result.ipa || 'N/A'}/</span>
                </div>
              </div>
              ...
              {result.notes && (
                <div className="p-2.5 bg-indigo-50/50 rounded-lg border border-indigo-100">
                  ...
                  <p className="text-xs text-slate-700 mt-1 leading-relaxed">{result.notes}</p>
                </div>
              )}
            </div>
          )}
          ...
          {result && activeTab === 'etymology' && (
            <div className="space-y-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
              <div className="flex items-center justify-between pb-2 border-b border-slate-200">
                ...
                <span className="text-xs font-bold text-indigo-700">{result.language}</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">Word</span>
                  <span className="text-sm font-bold text-slate-900">{result.word}</span>
                </div>
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">Proto-Root</span>
                  <span className="text-sm font-mono text-indigo-600">{result.root || 'N/A'}</span>
                </div>
                <div className="p-2.5 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase">IPA</span>
                  <span className="text-sm font-mono text-slate-700">{result.ipa || 'N/A'}</span>
                </div>
              </div>

              {result.etymology && (
                <div className="p-3 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase mb-1">Historical Origin & Evolution</span>
                  <p className="text-xs text-slate-700 leading-relaxed">{result.etymology}</p>
                </div>
              )}

              {result.cognates && result.cognates.length > 0 && (
                <div className="p-3 bg-white rounded-lg border border-slate-200">
                  <span className="text-[10px] font-bold text-slate-400 block uppercase mb-1.5">Cognates in Related Languages</span>
                  <div className="flex flex-wrap gap-1.5">
                    {result.cognates.map((cog: string, idx: number) => (
                      <span key={idx} className="text-xs bg-indigo-50 text-indigo-800 px-2 py-0.5 rounded-md border border-indigo-100 font-medium">
                        {cog}
                      </span>
                    ))}
                  </div>
